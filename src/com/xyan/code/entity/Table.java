...
public class Table {
	...
	public Table(String tableName) {
		this.tableName = tableName;//表名
		this.domainClassName = tableName;//实体类名称
		this.l_domainClassName=com.xyan.code.util.StringUtils.toLowerCaseFirstOne(domainClassName);//首字母小写的类名
	...
	public void calPackageName(String common){
		...
		this.domainPackageName=config.getProperty(
				"projectPackage")+"."+common+".model";
		this.servicePackageName=config.getProperty(
				"projectPackage")+"."+common+".service";
		this.mapperInterfacePackageName = config.getProperty(
				"projectPackage")+"."+common+".dao";
		this.controllerPackageName = config.getProperty(
				"projectPackage")+"."+common+".controller";
		this.voPackageName =config.getProperty(
	...
	private String parseDataType(String colType, String digits) {
		...
		if ("VARCHAR".equalsIgnoreCase(colType)) {
			...
			if ("0".equals(digits)||digits==null) {
				ct = TYPE_LONG;
			} else {
				ct = TYPE_FLOAT;
			}
			...
			ct = TYPE_INTEGER;
		} else if ("DATETIME".equalsIgnoreCase(colType)||"DATE".equalsIgnoreCase(colType) || "TIMESTAMP".equalsIgnoreCase(colType)) {
			ct = "Date";
		} else if ("BLOB".equalsIgnoreCase(colType)) {
			ct = "Object";
		} else if("INT".equalsIgnoreCase(colType)||"tinyint".equalsIgnoreCase(colType)){
			ct = TYPE_INTEGER;
